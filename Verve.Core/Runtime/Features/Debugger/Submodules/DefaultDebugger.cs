...
namespace Verve.Debug
{
    ...
    public sealed partial class DefaultDebugger : DebuggerSubmodule, IDrawableSubmodule
    {
        ...
        private struct ConsoleMessage
        {
            ...
            public ConsoleMessage(string text, LogType level = LogType.Log)
            {
                Text = text;
                Level = level;
...
// #endif
        ...
        private void ExecuteConsoleInput()
        {
            ...
            {
                AddToConsoleOutput($"命令执行错误: {ex.Message}", LogType.Error);
            }
        ...
        private void AddToConsoleOutput(string message, LogType level = LogType.Log)
        {
        ...
        ///   <para>消息颜色</para>
        /// </returns>
        private Color GetMessageColor(LogType level)
        {
            switch (level)
            {
