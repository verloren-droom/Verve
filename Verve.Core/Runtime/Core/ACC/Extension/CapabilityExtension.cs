namespace Verve
{
    ...
    public static class CapabilityExtension
    {
        ...
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void MarkActorDirty(this Capability self)
            => self?.OwnerWorld.Capabilities.MarkActorDirty(self.OwnerActor);

        /// <summary>
        ///   <para>手动标记指定组件为脏数据</para>
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void MarkComponentDirty<T>(this Capability self)
            where T : struct, IComponent
            => self?.OwnerWorld.MarkComponentDirty<T>(self.OwnerActor);
    }
}