...
namespace VerveEditor
{
    ...
    internal class EventBusDebuggerWindow : EditorWindow
    {
        ...
        {
            EventBus.OnEventRecorded += OnEventRecorded;
        }
        ...
        {
            if (EventBus.StringToHashCache != null)
            {
                foreach (var kvp in EventBus.StringToHashCache)
                {
        ...
        private List<EventRecordGroup> GetEventGroups()
        {
            ...
            foreach (var kvp in EventBus.Handlers)
            {
            ...
            {
                if (EventBus.Handlers.TryGetValue(GetEventHashFromKey(group.EventKey), out var activeHandlers))
                {
        ...
        private int GetEventHashFromKey(string eventKey)
        {
            ...
            if (EventBus.StringToHashCache != null && EventBus.StringToHashCache.TryGetValue(eventKey, out var hashValue))
            {
                return hashValue;
            }
