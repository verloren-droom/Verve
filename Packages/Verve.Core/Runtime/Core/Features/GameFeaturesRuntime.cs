    ...
    public sealed class GameFeaturesRuntime
    {
        ...
        public void LoadFeature([NotNull] string featureName, Action<bool> onComplete = null)
        {
            ...
            try
            {
                ...
                info.Feature.Load(new FeatureDependencies(CollectDependencies(featureName)));
        ...
        public void ActivateFeature([NotNull] string featureName)
        {
            ...
            try
            {
                ...
                info.State = FeatureState.Activated;
            }
            catch (Exception e)
            {
                info.State = FeatureState.Loaded;
        ...
        {
            if (!m_Features.TryGetValue(featureName, out var info) || info.State != FeatureState.Activated) return;
        ...
        public void UnloadFeature([NotNull] string featureName)
        {
            ...
            try
            {
                if (info.State == FeatureState.Activated)
        ...
        public void ReloadFeature(string featureName)
        {
        ...
        private IReadOnlyDictionary<string, IGameFeature> CollectDependencies(string featureName)
        {
            var dependencies = new Dictionary<string, IGameFeature>();
    
            if (!m_Dependencies.TryGetValue(featureName, out var depNames)) 
                return dependencies;

            for (int i = 0; i < depNames.Count; i++)
            {
                var depName = depNames[i];
                if (!m_Features.TryGetValue(depName, out var depInfo) || depInfo.State < FeatureState.Loaded)
                {
                    throw new GameFeatureMissingDependencyException(featureName, depName);
                }
        
                dependencies[depName] = depInfo.Feature;
            }
        ...
        {
            foreach (var kv in m_Features)
                if (kv.Value.Feature is TFeature t && kv.Value.State == FeatureState.Activated)
                    return t;
            return null;
        }
