...
namespace Verve.UniEx.Storage
{
    ...
    public abstract class StorageSubmodule : GameFeatureSubmodule<StorageGameFeatureComponent>, IStorage
    {
        ...
        public virtual async Task WriteDataAsync<TData>(string filePath, string key, TData value, Encoding encoding, IStorage.SerializerDelegate serializer, IStorage.DeserializerDelegate<TData> deserializer, CancellationToken ct = default)
        {
            try
            {
                await Task.Run(() =>
